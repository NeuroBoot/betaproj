document.addEventListener('DOMContentLoaded', async () => {
    ...
    // accuracy of sys 
    async function loadSysAccuracy() {
    try {
        const attRes = await fetch(`${API_BASE_URL}/attendance`, {
            headers: { 'Authorization': `Bearer ${token}`, 'Accept': 'application/json' }
        });

        const attendanceResult = await attRes.json();
        ...
        const el = document.getElementById('systemAccuracy');
        if (el) el.textContent = `${accuracyRate}%`;
